...
| Type | When | Quantity Change | On-Hand Change | Inbound Change | Allocated Change |
|------|------|-----------------|----------------|----------------|------------------|
| `ORDER_ALLOCATED` | Order submitted | 0 | No | No | +qty |
| `ORDER_FULFILLED` | Order fulfilled | -qty | -qty | No | -qty |
...
**Cancellation**: Can occur from draft or submitted → `syncOrderCancellation()`
...
5. **No Automated Reordering**
   - Manual reordering only
   - Future: Auto-generate POs based on thresholds + lead times

6. **No Advanced Forecasting**
   - Basic velocity calculation only
