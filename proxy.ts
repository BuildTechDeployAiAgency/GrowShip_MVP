 ...
 * The middleware.ts file is deprecated and will be removed in a future version.
 * 
 * Migration notes:
 * - Renamed from middleware.ts to proxy.ts
 * - Function renamed from `middleware` to `proxy`
 * - Same functionality, but Next.js recommends using this only for routing/rewrites
 * - Authentication checks should ideally be handled in route handlers or server components
 * 
 * To migrate: Run `npx @next/codemod@canary middleware-to-proxy .`
 */
...
export async function proxy(request: NextRequest) {
  ...
  if (
    ...
    try {
      ...
      if (profileError) {
        console.error("[Proxy] Error checking profile:", {
          userId: user.id,
      ...
      console.error("[Proxy] Unexpected error checking profile:", {
        userId: user.id,
