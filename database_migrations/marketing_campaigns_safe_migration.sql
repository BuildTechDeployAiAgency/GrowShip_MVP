-- =============================================
-- SAFE MARKETING CAMPAIGNS MIGRATION SCRIPT
...
-- Step 4A: Create utility functions (needed for migration)
DO $$ BEGIN RAISE NOTICE 'Creating utility functions...'; END $$;
...
BEGIN
  ...
  -- Ensure uniqueness - but only check if table exists
...
-- Step 4B: Create comprehensive marketing_campaigns table
        ...
        IF record_count > 0 THEN
            ...
            EXECUTE format('
                ...
                SELECT 
                    ...
                    start_date,
                    end_date,
                    COALESCE(budget, total_budget, 0),  -- Map old budget field
                    COALESCE(budget, total_budget, 0),  -- Use same value for allocated
                    0,  -- Start with 0 spent budget
                    COALESCE(status, ''draft''),
                    created_by,
                    created_at,
                    updated_at,
                    -- Defaults for new required fields
                    ''digital'',  -- Default channel
                    ''pending'',  -- Default approval status
                    COALESCE(budget, total_budget, 0),  -- Default brand contribution to full budget
                    0,  -- Default distributor contribution
                    ''brand_direct''  -- Default fund source
                FROM %I
                WHERE name IS NOT NULL  -- Ensure required fields exist
            ', backup_table_name);
...
DO $$ BEGIN RAISE NOTICE 'Setting up Row Level Security...'; END $$;
...
-- Step 9: Utility functions completed earlier
